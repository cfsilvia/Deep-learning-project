...
def point_annotator(
   ...
   # features = {'label': np.empty(3, dtype=int)}
    ...
    arrayModel = [[0,100,-1],[0,200,-1],[0,300,-1],[0,400,-1],[0,500,-1],[0,600,-1],[0,700,-1],[0,800,-1]]
    points= arrayModel[0:len(labels)]
   # points = np.array([[0,100,-1],[0,200,-1],[0,300,-1],[0,400,-1]])
   
    # for index in range(1,SHAPE_STACK[0]):
    #    points =  np.append(points,np.array([[index,-1,-1],[index,-1,-1],[index,-1,-1]]))
...
def arrange(shape,labels,points,rect,PATH_FOLDER):
    ...
    object = RearrangeData.HelperFunctions()
    object.GetRectangleInf(r)
    object.GetPointsInf(labels,points,LABELS)
    object.FusionData(PATH_FOLDER)
    object.ConverionPandastoText(PATH_FOLDER,shape)
...
def CreateListBox():
   ...
   values = ['nose','ear_Left', 'ear_Right', 'shoulders', 'center', 'hips', 'tail_Base', 'tail_End']
   for val in values:
...
def main():
    ...
    #labels=['ear_l', 'ear_r', 'tail','center']
    print(filenames)
    point_annotator(im_path, labels = LABELS)
